				...
				unsigned long arg)
{
	...
	mif_info("exynos_smc input: 0x%X, 0x%X, 0x%X, 0x%X\n", SMC_ID_CLK,
		SSS_CLK_ENABLE, 0, 0);
	err = exynos_smc(SMC_ID_CLK, SSS_CLK_ENABLE, 0, 0);
	mif_info("exynos_smc output: 0x%X\n", err);

	mif_info("exynos_smc input: 0x%X, 0x%X, 0x%X, 0x%X\n", SMC_ID,
		msr.mode, (unsigned int)param2, (unsigned int)param3);
