...
module_param(smm_output_filter, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(smm_output_filter,
		 "Output filter for the shared memory manager");
...
static int slsi_sm_bt_service_cleanup(bool allow_service_stop)
{
	...
	if (NULL != bt_service.service) {
		...
		if (0 != bt_service.debug_output_ref) {
			scsc_mx_service_mifram_free(bt_service.service,
					bt_service.debug_output_ref);
			bt_service.debug_output_ref = 0;
		}
		...
		scsc_mx_service_close(bt_service.service);
...
int slsi_sm_bt_service_start(void)
{
	...
	if (smm_output_filter) {
		/* Get shared memory region for the debugging information
		   from the MIF */
		err = scsc_mx_service_mifram_alloc(bt_service.service,
						   BSMHCP_DEBUG_POOL_SIZE,
						   &bt_service.debug_output_ref,
						   BSMHCP_ALIGNMENT);
		if (err)
			SCSC_TAG_WARNING(BT_COMMON, "mifram alloc failed\n");
	...
	err = scsc_bt_shm_init(smm_output_filter);
	if (err) {
				   ...
				   enum scsc_module_client_reason reason)
{
	...
	if (reason == SCSC_MODULE_CLIENT_REASON_RECOVERY && recovery_in_progress) {
		...
		mutex_lock(&bt_start_mutex);
		if (slsi_sm_bt_service_stop() == -EIO)
			SCSC_TAG_INFO(BT_COMMON, "Service stop failed during recovery.\n");
...
static int slsi_bt_service_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "  mailbox_debug_read            = %u\n",
		bt_service.mailbox_debug_read);
	seq_printf(m, "  mailbox_debug_write           = %u\n\n",
		bt_service.mailbox_debug_write);
	...
	seq_printf(m, "  interrupt_debug_count         = %zu\n\n",
...
static int __init scsc_bt_module_init(void)
{
	...
	init_waitqueue_head(&bt_service.debug_wait);
