...
static char debug_output[BSMHCP_DEBUG_STRING_MAX + 1];
...
static void scsc_bt_shm_irq_handler(int irqbit, void *data)
{
	...
	/* Wake the debug thread to dump debugging output */
	if (bt_service.bsmhcp_protocol->header.mailbox_debug_write !=
	    bt_service.bsmhcp_protocol->header.mailbox_debug_read) {
		bt_service.interrupt_debug_count++;

		wake_up(&bt_service.debug_wait);
	}
...
static ssize_t scsc_bt_shm_h4_acl_write(const unsigned char *data, size_t count)
{
	...
	if (BSMHCP_HAS_ROOM(tr_write, tr_read, BSMHCP_TRANSFER_RING_ACL_SIZE)) {
		...
		if ((td->flags & BSMHCP_ACL_PB_FLAG_MASK) == BSMHCP_ACL_PB_FLAG_START_NONFLUSH ||
			...
			/* data+4 to skip the HCI header, to align offsets with the rx detection */
			scsc_avdtp_detect_tx(td->hci_connection_handle, data+4, td->length);
...
static ssize_t scsc_hci_evt_read(char __user *buf, size_t len)
{
	...
	if (0 == bt_service.read_offset) {
		SCSC_TAG_INFO(BT_RX, "HCI Event [type=%s (0x%02x), length=%u]\n",
			scsc_hci_evt_decode_event_code(td->data), td->data[0], td->data[1]);
		...
		if (0 == ret) {
			...
			if (bt_service.read_offset == (sizeof(h4_acl_header) + td->length)) {
				...
				if (td->packet_boundary == HCI_ACL_PACKET_BOUNDARY_START_FLUSH)
					scsc_avdtp_detect_rx(td->hci_connection_handle, td->data, td->length);
			}
		} else {
			SCSC_TAG_WARNING(BT_H4, "copy_to_user returned: %zu\n", ret);
			ret = -EACCES;
...
#ifdef CONFIG_SCSC_PRINTK
static const char * const s_lnk_type_to_str[] = {
	"SCO",
	"ACL",
	"SETUP_ID",
	"SETUP_FHS",
...
static const char * const s_event_type_to_str[] = {
	"SUCCESS",
	"SYNC_TIMEOUT",
	"BUFFER_ERROR",
	"MEMBER_ERROR",
	...
	"BUFFER_OVERRUN",
	"BUFFER_DISCARD",
	"WBREE_AD_HEAD_PACKET",
	"WBREE_AD_ADDR_PACKET",
};

static const char *const s_acl_packet_type_to_str[] = {
	"NULL",
	"POLL",
	"FHS",
	"DM1",
...
static const char *const s_acl_23_packet_type_to_str[] = {
	"NULL",
	"POLL",
	"FHS",
	"DM1",
...
static char binary_output[65];
static char inttohex[16] = "0123456789ABCDEF";

#endif

void scsc_bt_shm_debug(void)
{
	while (bt_service.mailbox_debug_read != bt_service.mailbox_debug_write) {
		struct BSMHCP_DEBUG *debug = (struct BSMHCP_DEBUG *)&bt_service.debug_ptr[bt_service.mailbox_debug_read];

		/* Is the length valid? */
		if (0 == debug->length) {
			/* Data is corrupted by the firmware. Advance the read index to write */
			SCSC_TAG_ERR(BT_FW, "BSMHCP_DEBUG failed (length=0, read=%u, write=%u)\n", bt_service.mailbox_debug_read, bt_service.mailbox_debug_write);
			/*SCSC_BIN_DEBUG(debug, write - read);*/
			bt_service.mailbox_debug_read = bt_service.mailbox_debug_write;
		} else {
			u32 len = debug->length - sizeof(struct BSMHCP_DEBUG);

			if (debug->length + bt_service.mailbox_debug_read > bt_service.mailbox_debug_write) {
				SCSC_TAG_ERR(BT_FW, "BSMHCP_DEBUG failed (type=%u, length=%u, read=%u, write=%u)\n",
					     debug->type, debug->length, bt_service.mailbox_debug_read,
					     bt_service.mailbox_debug_write);
				bt_service.mailbox_debug_read = bt_service.mailbox_debug_write;
				break;
			}

			/* Determine the debug event and process it */
			switch (debug->type) {
			case BSMHCP_DEBUG_INIT:
				SCSC_TAG_DEBUG(BT_FW, "BSMHCP_DEBUG_INIT\n");
				bt_service.mailbox_debug_read += debug->length;
				break;
			case BSMHCP_DEBUG_WRAP:
				SCSC_TAG_DEBUG(BT_FW, "BSMHCP_DEBUG_WRAP\n");
				bt_service.mailbox_debug_read = 0;
				break;
			case BSMHCP_DEBUG_STRING:
				/* Output string to logring */
				memcpy(debug_output, (char *)(debug + 1), len);
				debug_output[len] = 0;
				SCSC_TAG_INFO(BT_FW, "%s", debug_output);
				bt_service.mailbox_debug_read += debug->length;
				break;
#ifdef CONFIG_SCSC_PRINTK
			case BSMHCP_DEBUG_FROM_AIR:
			case BSMHCP_DEBUG_TO_AIR:
			{
				struct BSMHCP_DEBUG_AIR_PACKET *air_packet = (struct BSMHCP_DEBUG_AIR_PACKET *)&bt_service.debug_ptr[bt_service.mailbox_debug_read];
				enum bt_link_type_enum lnk_type = (enum bt_link_type_enum) air_packet->lnk_type;
				const char *pkt_type;

				switch (lnk_type) {
				case BT_LINK_TYPE_ACL:
				{
					pkt_type = s_acl_packet_type_to_str[air_packet->pkt_type & 0x0F];
					break;
				}
				case BT_LINK_TYPE_ACL_23:
				{
					pkt_type = s_acl_23_packet_type_to_str[air_packet->pkt_type & 0x0F];
					break;
				}
				default:
				{
				...
				if (BT_LINK_TYPE_ACL == lnk_type || BT_LINK_TYPE_ACL_23 == lnk_type)
					if (air_packet->pkt_type > 1)
						SCSC_TAG_INFO(BT_FW, "(%9u) %s[%02u:%u:%s] (event=%s, pkt=%s, size=%u, flow=%u)\n",
							      air_packet->timestamp,
							      air_packet->type == BSMHCP_DEBUG_FROM_AIR ? "FROM_AIR" : "TO_AIR",
							      air_packet->channel,
							      air_packet->link_seqn,
							      air_packet->link_arqn ? "ACK" : "NAK",
							      s_event_type_to_str[air_packet->event_type],
							      pkt_type,
							      air_packet->payload_num_data_bytes,
							      air_packet->link_flow);
					else
						SCSC_TAG_INFO(BT_FW, "(%9u) %s[%02u:%s] (event=%s, pkt=%s, flow=%u)\n",
							      air_packet->timestamp,
							      air_packet->type == BSMHCP_DEBUG_FROM_AIR ? "FROM_AIR" : "TO_AIR",
							      air_packet->channel,
							      air_packet->link_arqn ? "ACK" : "NAK",
					...
					SCSC_TAG_INFO(BT_FW, "(%9u) %s[%02u] (type=%s, event=%s)\n",
						      air_packet->timestamp,
						      air_packet->type == BSMHCP_DEBUG_FROM_AIR ? "FROM_AIR" : "TO_AIR",
						      air_packet->channel,
						      s_lnk_type_to_str[air_packet->lnk_type],
						      s_event_type_to_str[air_packet->event_type]);

				bt_service.mailbox_debug_read += debug->length;
				break;
			}
			case BSMHCP_DEBUG_SPEEDY:
			{
				struct BSMHCP_DEBUG_SPEEDY_PACKET *speedy_packet = (struct BSMHCP_DEBUG_SPEEDY_PACKET *)&bt_service.debug_ptr[bt_service.mailbox_debug_read];

				SCSC_TAG_INFO(BT_FW, "SPEEDY[%s] (addr=0x%08x, data=%08x, done=%s, error=%s, timeout=%s, operation=%s)\n",
					      speedy_packet->flags & BSMHCP_DEBUG_SPEEDY_FLAG_FOREGROUND ? "M4" : "R4",
					      speedy_packet->address,
					      speedy_packet->data,
					      speedy_packet->flags & BSMHCP_DEBUG_SPEEDY_FLAG_APB_CYCLE_DONE ? "y" : "n",
					      speedy_packet->flags & BSMHCP_DEBUG_SPEEDY_FLAG_APB_CYCLE_ERR ? "y" : "n",
					      speedy_packet->flags & BSMHCP_DEBUG_SPEEDY_FLAG_TIMEOUT_ERROR ? "y" : "n",
					      speedy_packet->flags & BSMHCP_DEBUG_SPEEDY_FLAG_WRITE ? "write" : "read");

				bt_service.mailbox_debug_read += debug->length;
				break;
			}
			case BSMHCP_DEBUG_BINARY_DATA:
			{
				struct BSMHCP_DEBUG_BINARY_DATA_PACKET *binary_data_packet = (struct BSMHCP_DEBUG_BINARY_DATA_PACKET *)&bt_service.debug_ptr[bt_service.mailbox_debug_read];
				u32                                    block, offset, block_size, block_offset, block_length;
				u8                                     *block_data = (u8 *)(binary_data_packet + 1);
				u32                                    length = binary_data_packet->length - sizeof(struct BSMHCP_DEBUG_BINARY_DATA_PACKET);

				block_size = 32;
				for (block = 0; block < length; block += block_size) {
					block_offset = block * block_size;
					block_length = (length - block_offset) >= block_size ?
						       block_size : length - block_offset;
					for (offset = 0; offset < block_length; offset++) {
						binary_output[(offset << 1) + 0] = inttohex[(block_data[block_offset + offset] & 0xF0) >> 4];
						binary_output[(offset << 1) + 1] = inttohex[(block_data[block_offset + offset] & 0x0F)];
					}
					binary_output[offset] = 0;

					SCSC_TAG_INFO(BT_FW, "(%u) ID:%u [%04x: %s]\n",
						      binary_data_packet->timestamp,
						      binary_data_packet->identifier,
						      block,
						      binary_output);
				}
			...
			default:
				...
				SCSC_TAG_DEBUG(BT_FW, "Bluetooth debug msg (type: 0x%04x, length: %u, read: %u, write: %u)\n",
					       debug->type, debug->length, bt_service.mailbox_debug_read, bt_service.mailbox_debug_write);
				bt_service.mailbox_debug_read += debug->length;
				break;
			}
		}
...
static void scsc_bt_reset_avdtp_detection(void)
{
	bt_service.avdtp_hci_connection_handle = 0;
	bt_service.avdtp_signaling_src_cid = 0;
	bt_service.avdtp_signaling_dst_cid = 0;
	bt_service.avdtp_streaming_src_cid = 0;
	bt_service.avdtp_streaming_dst_cid = 0;
	bt_service.bsmhcp_protocol->header.avdtp_detect_stream_id = 0;
}
...
static ssize_t scsc_bt_shm_h4_read_hci_evt(char __user *buf, size_t len)
{
	...
	while (BT_READ_OP_NONE == bt_service.read_operation && 0 == ret && !bt_service.hci_event_paused && bt_service.mailbox_hci_evt_read != bt_service.mailbox_hci_evt_write) {
		...
		if (td->event_type & BSMHCP_EVENT_TYPE_CONNECTED) {
			...
			if (bt_service.avdtp_hci_connection_handle == td->hci_connection_handle) {
				scsc_bt_reset_avdtp_detection();
				mmiowb();
			}
...
ssize_t scsc_bt_shm_h4_read(struct file *file, char __user *buf, size_t len, loff_t *offset)
{
	...
	while (0 == atomic_read(&bt_service.error_count) && 0 == consumed) {
		...
		SCSC_TAG_DEBUG(BT_H4, "read_operation=%u, hci_event_paused=%u, acldata_paused=%u, debug_read=%u, debug_write=%u\n",
			       bt_service.read_operation, bt_service.hci_event_paused,
			       bt_service.acldata_paused, bt_service.mailbox_debug_read, bt_service.mailbox_debug_write);
...
static int scsc_bt_shm_thread(void *data)
{
	int ret = 0;

	bt_service.mailbox_debug_write = bt_service.bsmhcp_protocol->header.mailbox_debug_write;

	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(bt_service.debug_wait,
					       bt_service.bsmhcp_protocol->header.mailbox_debug_write !=
					       bt_service.bsmhcp_protocol->header.mailbox_debug_read ||
					       bt_service.debug_terminate);

		/* Is this a closure of the thread */
		if (bt_service.debug_terminate)
			break;

		/* Update the local write pointer */
		bt_service.mailbox_debug_write = bt_service.bsmhcp_protocol->header.mailbox_debug_write;

		/* Process any debugging information received from the firmware */
		if (bt_service.mailbox_debug_read != bt_service.mailbox_debug_write) {
			scsc_bt_shm_debug();
			bt_service.bsmhcp_protocol->header.mailbox_debug_read = bt_service.mailbox_debug_read;
		}

		scsc_service_mifintrbit_bit_set(bt_service.service, bt_service.bsmhcp_protocol->header.ap_to_bg_int_src, SCSC_MIFINTR_TARGET_R4);
	}

	complete(&bt_service.debug_thread_complete);
...
int scsc_bt_shm_init(u32 filter)
{
	...
	bt_service.bsmhcp_protocol->header.smm_output_filter = filter;
	...
	/* Initialise the debug system if region is available */
	if (bt_service.debug_output_ref != 0) {
		bt_service.debug_ptr = (char *)scsc_mx_service_mif_addr_to_ptr(bt_service.service, bt_service.debug_output_ref);
		bt_service.bsmhcp_protocol->header.debug_output_offset = bt_service.debug_output_ref;
	}

	/* Initialise the interrupt handlers */
	scsc_bt_shm_init_interrupt();

	if (bt_service.debug_ptr) {
		init_completion(&bt_service.debug_thread_complete);
		bt_service.debug_thread = kthread_run(scsc_bt_shm_thread, NULL, "scsc_bt_shm_thread");
	}
...
{
	/* Stop the outputting thread */
	if (bt_service.debug_thread) {
		bt_service.debug_terminate = true;
		kthread_stop(bt_service.debug_thread);
		wake_up_interruptible(&bt_service.debug_wait);
		wait_for_completion(&bt_service.debug_thread_complete);
		bt_service.debug_thread = NULL;
	}
}
...
void scsc_bt_shm_exit(void)
{
	...
	bt_service.hci_event_paused = false;
	bt_service.acldata_paused = false;
	bt_service.debug_ptr = NULL;
	bt_service.bsmhcp_protocol = NULL;
	bt_service.debug_terminate = false;
