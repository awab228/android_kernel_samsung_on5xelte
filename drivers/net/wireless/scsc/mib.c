...
{
	u8 *newBuffer = kmalloc(dst->dataLength + bufferLength, GFP_KERNEL);
...
u16 slsi_mib_encode(struct slsi_mib_data *buffer, struct slsi_mib_entry *value)
{
	...
	u8     *tmpBuffer = kmalloc(requiredSize, GFP_KERNEL);
...
int slsi_mib_encode_get_list(struct slsi_mib_data *buffer, u16 psidsLength, const struct slsi_mib_get_entry *psids)
{
	...
	buffer->dataLength = 0;
	buffer->data = kmalloc((u32)(psidsLength * 13), GFP_KERNEL); /* 13 Bytes per get will be loads of space for the max 3 indexes */
	if (!buffer->data) {
		SLSI_ERR_NODEV("kmalloc(%d) failed\n", psidsLength * 13);
