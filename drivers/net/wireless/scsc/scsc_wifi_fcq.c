...
uint scsc_wifi_fcq_smod = 200;
module_param(scsc_wifi_fcq_smod, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(scsc_wifi_fcq_smod, "Initial value of unicast smod - peer normal (default = 128)");
...
uint scsc_wifi_fcq_qmod = 200;
module_param(scsc_wifi_fcq_qmod, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(scsc_wifi_fcq_qmod, "Initial value of unicast qmod - peer normal (default = 200)");
...
#define STOP_GUARD_GMOD		10 /* Current hip4 DAT space is 218 */
#define STOP_GUARD_SMOD		0
#define STOP_GUARD_QMOD		1
...
#endif
static atomic_t in_pause_state;
static DEFINE_SPINLOCK(gbot_lock);
...
static inline bool is_in_pause(void)
{
	return atomic_read(&in_pause_state);
}
...
static inline void fcq_wake_all_queues(struct slsi_dev *sdev)
{
	...
	list_for_each_entry_safe(pc_node, next, &peers_cache_list, list) {
		...
		for (i = 0; i < SLSI_NETIF_Q_PER_PEER; i++) {
			if (is_gmod_active(sdev) && is_smod_active(pc_node->qs) && is_qmod_active(&pc_node->qs->ac_q[i]) && !is_in_pause()) {
...
void scsc_wifi_fcq_pause_queues(struct slsi_dev *sdev)
{
	...
	atomic_set(&in_pause_state, 1);
	fcq_stop_all_queues(sdev);
	spin_unlock(&gbot_lock);
}
...
#endif
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Stop subqueue vif: %d peer: %d ac: %d qcod (%d) qmod (%d)\n", vif, peer_index, priority, atomic_read(&queue->qcod), atomic_read(&queue->qmod));
...
/* Setting ENABLE_CONTROLLED_PORT_STATE_LOGIC allows the FCQ code to act on the current
 * state. This is disabled because at time of writing, the state is "blocked" when
 * EAPOL messages are being sent - and therefore the messages are not passed to the
 * chip. Remove this comment when fixed.
 * #define ENABLE_CONTROLLED_PORT_STATE_LOGIC 1
 */
...
#ifdef ENABLE_CONTROLLED_PORT_STATE_LOGIC
	/* Need to debug blocked state behaviour - EAPOL is blocked. */
	if (qs->controlled_port_state == SCSC_WIFI_FCQ_8021x_STATE_BLOCKED) {
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "8021x_STATE_BLOCKED\n");
...
static int fcq_receive_smod_domain(struct net_device *dev, struct scsc_wifi_fcq_data_qset *qs, struct slsi_dev *sdev, u16 priority, u8 peer_index, u8 vif)
{
	...
	if (!is_smod_active(qs) && (scod + SCSC_WIFI_FCQ_SMOD_RESUME_HYSTERESIS/total < atomic_read(&qs->smod))) {
		...
		for (i = 0; i < SLSI_NETIF_Q_PER_PEER; i++) {
			if (is_gmod_active(sdev) && is_qmod_active(&qs->ac_q[i]) && !is_in_pause())
...
static int fcq_receive_qmod_domain(struct net_device *dev, struct scsc_wifi_fcq_data_qset *qs, struct slsi_dev *sdev, u16 priority, u8 peer_index, u8 vif)
{
	...
	if (!is_qmod_active(&qs->ac_q[priority]) && ((qcod + SCSC_WIFI_FCQ_QMOD_RESUME_HYSTERESIS/total) < atomic_read(&queue->qmod))) {
		...
		if (is_gmod_active(sdev) && is_smod_active(qs) && !is_in_pause())
...
#endif
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Start subqueue vif: %d peer: %d ac: %d qcod (%d) qmod (%d)\n", vif, peer_index, priority, atomic_read(&queue->qcod), atomic_read(&queue->qmod));
...
static int fcq_data_q_init(struct net_device *dev, enum scsc_wifi_fcq_queue_set_type type, struct scsc_wifi_fcq_data_q *queue,
...
static void fcq_qset_init(struct net_device *dev, enum scsc_wifi_fcq_queue_set_type type, struct scsc_wifi_fcq_data_qset *qs, u8 qs_num)
{
	int                         i;
	...
	for (i = 0; i < SLSI_NETIF_Q_PER_PEER; i++) {
		...
		qs->ac_inuse &= ~(1 << i);
		queue = &qs->ac_q[i];
		fcq_data_q_init(dev, type, queue, qs, qs_num, i);
	}
	/* Give all qmod to BE */
	qs->ac_inuse = 1;
}
...
#endif
{
	...
	fcq_qset_init(dev, SCSC_WIFI_FCQ_QUEUE_SET_TYPE_UNICAST, qs, qs_num);
	SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Unicast queue set inited vif %d peer_index %d\n", vif, peer->aid);
#ifdef CONFIG_SCSC_WLAN_DEBUG
	...
	SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Add new peer qs %p vif %d peer->aid %d\n", qs, vif, peer->aid);
...
int scsc_wifi_fcq_multicast_qset_init(struct net_device *dev, struct scsc_wifi_fcq_data_qset *qs, struct slsi_dev *sdev, u8 vif)
{
	...
	fcq_qset_init(dev, SCSC_WIFI_FCQ_QUEUE_SET_TYPE_MULTICAST, qs, 0);
	SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Multicast queue set inited\n");
#ifdef CONFIG_SCSC_WLAN_DEBUG
	...
	SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Add Multicast Qset %p vif %d peer->aid 0\n", qs, vif);
...
void scsc_wifi_fcq_qset_deinit(struct net_device *dev, struct scsc_wifi_fcq_data_qset *qs, struct slsi_dev *sdev, u8 vif, struct slsi_peer *peer)
{
	...
	if (scod != 0)
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Data set (0x%p) deinit: scod is %d\n",
				qs, scod);
...
#endif
	if (peer)
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Delete qs %p vif %d peer->aid %d\n", qs, vif, peer->aid);
	else
		SLSI_DBG4_NODEV(SLSI_WIFI_FCQ, "Delete qs %p vif %d Multicast\n", qs, vif);
