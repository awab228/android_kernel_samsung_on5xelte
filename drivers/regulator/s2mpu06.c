...
static int buf_to_i2c_read(const char *buf)
{
	...
	ret = s2mpu06_read_reg(static_info->i2c, addr, &val);
	if (ret < 0)
		return ret;
...
static int buf_to_i2c_write(const char *buf)
{
	...
	ret = s2mpu06_write_reg(static_info->i2c, addr, val);
	if (ret < 0)
		return ret;
...
static int s2mpu06_pmic_suspend(struct device *dev)
{
	...
	s2mpu06_write_reg(s2mpu06->i2c, S2MPU06_PMIC_REG_B3CTRL2, 0x2A); /* 1.85V */
	return 0;
}
