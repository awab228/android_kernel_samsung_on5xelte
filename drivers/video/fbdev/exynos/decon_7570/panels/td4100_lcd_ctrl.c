...
static int dsim_panel_set_brightness(struct lcd_info *lcd, int force)
{
	...
	if (lcd->bl > 128)
		bl_reg[1] = (lcd->bl - 128) * 100 / 127 + 80;
	else if (lcd->bl >= 3)
		bl_reg[1] = (lcd->bl - 3) * 78 / 125 + 2;
	else
		bl_reg[1] = 0x00;
...
static int dsim_resume_reason(struct dsim_device *dsim)
{
	...
	/* this is temporary solution. 12 is finger print irq number from etspi-drdyPin */
	ret = dsim->resume_request | check_wakeup_reason(12);

	if (ret)
		dsim_info("%s: %d, %d\n", __func__, dsim->resume_request, check_wakeup_reason(12));
