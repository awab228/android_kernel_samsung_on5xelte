 ...
 */
static const char *cod9002x_dnc_hp_vol_enable_text[] = {"Off", "On"};
...
/**
  ...
  */
static const char *cod9002x_dnc_zcd_enable_text[] = {"Off", "On"};
	...
	switch (event) {
		...
		case SND_SOC_DAPM_PRE_PMD:
			...
			if (hp_on) {
				...
				if(cod9002x->dnc_hp_vol & DNC_HP_VOL_EN_MASK){
					    snd_soc_write(w->codec, COD9002X_30_VOL_HPL,
								cod9002x->vol_hpl);
						snd_soc_write(w->codec, COD9002X_31_VOL_HPR,
								cod9002x->vol_hpr);
				}else{
					    snd_soc_write(w->codec, COD9002X_30_VOL_HPL, 0x1A);
						snd_soc_write(w->codec, COD9002X_31_VOL_HPR, 0x1A);
				}
			}
			break;
		default:
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
		case SND_SOC_DAPM_PRE_PMU:
			...
			cod9002x->dnc_hp_vol = snd_soc_read(w->codec, COD9002X_56_DNC3);
...
static void cod9002x_jack_det_work(struct work_struct *work)
{
	...
	if (jackdet->jack_det && jackdet->mic_det){
		/* 4 Pole Jack-in */
		snd_soc_write(codec, 0x82, 0x97);
		snd_soc_write(codec, 0x97, 0x32);
	}else if(jackdet->jack_det){
		/* 3 Pole Jack-in */
		snd_soc_write(codec, 0x82, 0x97);
		snd_soc_write(codec, 0x97, 0x22);
	}else{
		/* Jack-out */
		snd_soc_write(codec, 0x82, 0x17);
...
static irqreturn_t cod9002x_threaded_isr(int irq, void *data)
{
	...
	stat1 = snd_soc_read(codec, COD9002X_07_STATUS1);
	...
	if (det_status_change) {
		queue_delayed_work(cod9002x->jack_det_wq,
				&cod9002x->jack_det_work, msecs_to_jiffies(cod9002x->mic_det_delay));
